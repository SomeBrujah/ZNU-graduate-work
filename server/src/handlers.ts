...
const handleGetUsers = async (connectionId: string) => {
  await postToConnection(
    ...
    JSON.stringify({
      type: "users",
      value: await getAllUsers()
    })
  )

  return responseOK;
}
// {"action":"getMessages","targetNickname":"John","limit":50}
// {"action":"getMessages","targetNickname":"Max","limit":50}
// {"action":"sendMessage","message":"Hello partner","recipientNickname":"Max"}
